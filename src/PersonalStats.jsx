...
export default function PersonalStats() {
    ...
    [`Studied Graphic Design from September 2021 to July 2022`,`I completed a short course through University of Cape Town, completed 3 courses on Udemy from March until May, and completed a training program from June until July with Brainnest`],
    [`Promoted to Lead FeedAi QA Specialist in August 2022 - until present`,`I maintained the same responsibilities as before, but now I also manage a team of FeedAi QA Specialists. I have worked on training the specialists, I update them on new projects, I look for any mistakes in the work, I check projects for trends of performance of the specialists as well as the projects themselves so I can find ways of improving work flow, I report back issues with either the projects or the Ai, I find ways of improving the Ai's database, and I run team meetings to boost team connectivity.`],
    [`Moved in January 2023`,`relocated to Porto, Portugal with my citizenship, and continuing with my work as a ‘digital nomad’`],
    [`Started studying Front-end Engineering in March 2023 and I am continuing at present`,`From March until September 2023 I studied and completed the Front-end Engineering Professional Course/Certification on Codecademy, I continued to work on minor personal projects until August 2024 when I started learning through the SmolJames.com, updating front-end engineering technologies, and starting back-en/full-stack too`]]
    ...
    const SkillHandler = (skillSet) =>{
        ...
        Object.keys(titleKeys).map((key, index)=>{
            ...
            fullComponent.push(<h3 key={index}>{key}</h3>);
            ...
            Object.entries(titleKeys[key]).map((skill, i)=>{
                ...
                skillArray.push(<p key={i}>{skill[0]}</p>);
                ...
                for(let j = 0; j < skill[1]; j++){
                    ...
                    stars.push(<i key={j} className="fa-solid fa-star"></i>)
                }
                ...
                skillArray.push(<div key={'stars ' + i} className='stars'>{stars}</div>);
                ...
                fullComponent.push(<div key={'array ' + index + i} className='skillArray'>{skillArray}</div>);
            })
        })
